...
For real-provider gated runs, follow the standard pattern from the [voice customer support how-to](/arena/how-to/voice/voice-customer-support/) — secret-gated `head.repo.full_name == github.repository` job, provider keys in `secrets:`.
