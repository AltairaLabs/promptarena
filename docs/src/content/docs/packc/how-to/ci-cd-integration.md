...
pipeline {
    ...
    stages {
        stage('Setup') {
            steps {
                sh 'go version'
                sh 'go install github.com/AltairaLabs/promptarena/v2/packc@latest'
                sh 'packc version'
            }
        }
...
jobs:
  build-packs:
    ...
    steps:
      ...
      - run:
          name: Install packc
          command: go install github.com/AltairaLabs/promptarena/v2/packc@latest
...
```yaml
- name: Install packc
  run: go install github.com/AltairaLabs/promptarena/v2/packc@v0.1.0
```
