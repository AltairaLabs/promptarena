...
// launcher's write; it is not a licence to swap themes mid-session.
var (
...
// Colors returns the resolved Theme (semantic colours) for the current
// session. Use it when a call site builds its own lipgloss.Style and only
// needs a colour — e.g. Foreground(theme.Colors().TextBody) — so the site
...
// cannot be read — matching the TUI's historical hardcoded-dark behaviour.
