...
func init() {
	...
	generateCmd.Flags().String("from-recordings", "",
		"Glob of local recordings: arena run output (out/*.json), PromptKit session recordings (*.recording.json, *.jsonl) or transcripts") // NOSONAR
	...
	generateCmd.Flags().String("task-type", "", "task_type to set on generated scenarios (default: conversation)") // NOSONAR
	generateCmd.Flags().String("pack", "", "Deprecated alias for --task-type")                                     // NOSONAR
	_ = generateCmd.Flags().MarkDeprecated("pack", "it only ever set task_type; use --task-type")
