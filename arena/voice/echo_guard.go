...
// If CouplingFactor is <= 0, it defaults to 0.5 (defaultCouplingFactor).
...
// Reset clears the speaking window and resets the playback estimate immediately.
// Callers should invoke it on barge-in (when queued playback is flushed): without
// it, frames already recorded before the flush would keep gating the mic for
// their full now-irrelevant duration, and a stale high playback RMS would carry
// over into subsequent utterances.
